...
def insert_products_cart(_id_p, user_id):
    ...
    try:
        cur = conn.cursor()
        cur.execute(f'''SELECT * FROM Carrito WHERE user_ID='{user_id}'
                    AND product_ID='{_id_p}' ''')
        if cur.fetchone():
            cur.execute(f'''UPDATE Carrito SET cantidad=cantidad+1
                        WHERE user_ID='{user_id}' AND product_ID='{_id_p}' ''')
            conn.commit()
            print("stoy en 167")
            return True
        else:
            cur.execute(f'''INSERT INTO Carrito (user_ID, product_ID, cantidad)
                        VALUES ( '{user_id}', '{_id_p}', '{1}')''')
            conn.commit()
            print("stoy en 171")
            return True
    except Error as e:
